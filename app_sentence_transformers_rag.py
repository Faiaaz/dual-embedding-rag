...
class SentenceTransformerRAG:
    def __init__(self, storage_file='documents_sentence_transformers.json', model_name='all-MiniLM-L6-v2'):
    ...
    def search(self, query: str, top_k: int = 3):
        """Search for relevant documents using SentenceTransformer similarity"""
...
# Initialize RAG system with SentenceTransformers
...
def health_check():
    ...
    return jsonify({
        'status': 'healthy',
        'message': 'SentenceTransformer RAG API is running',
        'documents_count': len(rag.documents),
        'method': 'SentenceTransformer with PDF, DOCX, TXT, CSV, and SQLite DB support',
...
def home():
    ...
    return jsonify({
        'message': 'Welcome to the SentenceTransformer RAG API!',
        'note': 'This version uses SentenceTransformers for better semantic understanding',
        'model_name': rag.model_name,
        'storage_file': rag.storage_file,
        'endpoints': {
